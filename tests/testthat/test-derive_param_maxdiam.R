...
temp <- input %>% group_by(USUBJID,FAOBJ,FASCAT,ATPTREF,FALNKGRP) %>%
  arrange(desc(AVAL), .by_group = TRUE) %>%
  summarise(AVAL = first(AVAL)) %>%
  mutate(FATEST = "Maximum Diameter",
         FATESTCD = "MAXDIAM",
         DTYPE = "MAXIMUM")
...
actual <- derive_param_maxdiam(
  ...
  by_vars = vars(USUBJID,FAOBJ,FALNKGRP),
  test_maxdiam = "Maximum Diameter",
  testcd_maxdiam = "MAXDIAM"
)

# testthat
testthat::test_that('Check if a new record is created with Maximum diameter value',
...
# testthat
testthat::test_that('Check if filter condition and test/testcd values works fine',
                    expect_dfs_equal(actual, expected, keys = c('USUBJID','FAOBJ','FALNKGRP',"FATPT"))
)
...
# Test 3: Check if there are multiple records with maximum diameter
#         within by group then the MAXDIAM record is identified correctly
# Expected Output
temp <- input %>% group_by(USUBJID, FAOBJ,FASCAT,ATPTREF,FALNKGRP) %>%
...
# testthat
testthat::test_that('Check if there are multiple records with maximum diameter
                    within by group then the MAXDIAM record is identified correctly',
