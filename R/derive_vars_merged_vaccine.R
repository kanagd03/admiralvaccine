...
#' passed on `ex_vars` arguement
#'
#' @param dataset_face Input dataset
#'              
#' @param dataset_ex EX dataset to merge with the input dataset
#' 
#' @param dataset_suppface Supplementary input dataset
#'               By default dataset_suppface will be NULL,user has to provide 
...
#'   dataset_face = face1,
#'   dataset_ex = ex1,
#'   dataset_suppface = suppface,
#'   dataset_suppex = NULL,
#'   ex_vars = vars(EXTRT,EXDOSE,EXSEQ)
#' )
 
derive_vars_merged_vaccine <- function(
  dataset_face,
  dataset_ex,
  dataset_suppface=NULL,
  dataset_suppex=NULL,
  ex_vars
  
){
  
  assert_data_frame(dataset_face, required_vars = NULL)
  assert_data_frame(dataset_suppface, optional = TRUE)
  ...
  if(!is.null(dataset_suppface)){
   dataset_face <- metatools::combine_supp(dataset_face,dataset_suppface)
  }
  ...
  if(!("FATPTREF" %in% names(dataset_face))) {
    dataset_face <- dataset_face %>% 
      mutate(FATPTREF='VACCINATION 1')}
  
  if(!("FALAT" %in% names(dataset_face))) {
    dataset_face <- dataset_face %>% 
      mutate(FALAT = '')}
  
  if(!("FADIR" %in% names(dataset_face))) {
    dataset_face <- dataset_face %>% 
      mutate(FADIR = '')}
  
  if(!("FALOC" %in% names(dataset_face))) {
    dataset_face <- dataset_face %>% 
      mutate(FALOC = '')}
  
  if(!("FAEVINTX" %in% names(dataset_face))) {
    dataset_face <- dataset_face %>% 
      mutate(FAEVINTX = '')}
  
  if(("FALNKGRP" %in% names(dataset_face))) {
    dataset_face <- dataset_face %>% 
  ...
  #rename EX variables  
  dataset_ex <- dataset_ex %>% 
    rename(FATPTREF=EXLNKGRP,FALOC=EXLOC,FALAT=EXLAT,FADIR=EXDIR)
  
  #Filter records for  ADMINISTRATION SITE events and merge it with EX dataset
  dataset_adminstration <- dataset_face %>% 
  ...
  face1 <- derive_vars_merged(
    ...
    new_vars = ex_vars,
    by_vars = vars(USUBJID,FATPTREF,FALOC,FALAT,FADIR)
  )
  
  #Filter records for  SYSTEMIC events and merge it with EX dataset
  dataset_systemic <- dataset_face %>% 
