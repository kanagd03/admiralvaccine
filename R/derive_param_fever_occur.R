...
#' 
#' Getting `FAOBJ = "FEVER"` records from `SDTM.VS` dataset, if there are no FEVER 
#' records in input dataset
#' 
...
#'  
#' output <- derive_param_fever_occurr(dataset = input,
#'                                    source_data = vs)
                               ...
                               faobj)
{
  ...
  if (x == 0)
  {
    ...
    fever <- source_data %>%
      ...
      mutate(FATEST		= "Occurrence Indicator",
             FATESTCD		= "OCCUR",
             FASCAT = "SYSTEMIC",
             FAOBJ		= faobj) %>%
